        ...
        {loadError ? (
          ...
          <div className="flex-1 overflow-y-auto grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 content-start">
            {filtered.map(p => (
              ...
              >
                <div className="w-8 h-8 bg-primary rounded-lg flex items-center justify-center mb-2 group-hover:bg-primary transition-colors">
                  <Package className="h-4 w-4 text-primary" />
                </div>
                <p className="text-sm font-semibold text-gray-900 truncate">{p.name}</p>
                <p className="text-xs text-gray-400 mt-0.5">Stock: {p.quantity}</p>
                <p className="text-base font-bold text-primary mt-1">{sym} {p.selling_price?.toFixed(2) || '0.00'}</p>
              </button>
            ))}
          </div>
        )}
      ...
      <div className="flex w-80 flex-col bg-muted/50 p-4">
        ...
        <div className="border-t border-gray-200 pt-4 space-y-2.5">
          ...
          <Button
            ...
            className="w-full bg-primary hover:bg-primary text-white h-10 gap-2"
          >
            {isCheckingOut ? (
