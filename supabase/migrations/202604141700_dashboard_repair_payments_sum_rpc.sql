-- Suma de cobros de reparación para el dashboard: misma visión que repairTicketsOrgScopeOr
-- (taller por organization_id + tickets legacy sin org pero creados por miembro activo).
-- Evita agregados PostgREST + RLS que pueden devolver 0 aunque los tickets se listen bien.

CREATE INDEX IF NOT EXISTS idx_payments_ticket_created_at
...
COMMENT ON FUNCTION public.get_dashboard_repair_payments_sum(uuid, timestamp with time zone, timestamp with time zone, uuid)
  IS 'Suma amount de payments con ticket en el período; alcance org (incl. tickets legacy) o modo solo (user_id del ticket y shop_owner_id).';
