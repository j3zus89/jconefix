...
/**
 * Sube el logo del taller al bucket `avatars` (carpeta = userId, misma política RLS que el avatar).
...
export async function uploadShopLogo(
  ...
  const path = `${userId}/shop-logo-${Date.now()}-${Math.random().toString(36).slice(2, 10)}.${ext}`;
