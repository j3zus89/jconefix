...
export function CapacitorBottomNav() {
  ...
  return (
    <>
      <nav
        className="fixed bottom-0 left-0 right-0 z-[100] flex border-t border-white/10 bg-[#004d40] pb-[max(0.35rem,env(safe-area-inset-bottom,0px))] shadow-[0_-4px_20px_rgba(0,0,0,0.25)] md:hidden overflow-x-auto"
        ...
        {NAV.map(({ href, label, icon: Icon, match }) => {
          ...
          return (
            <Link
              ...
              href={href}
              className={cn(
                'flex min-h-[52px] min-w-[80px] flex-1 flex-col items-center justify-center gap-0.5 px-1 py-1.5 text-[10px] font-semibold transition-colors active:opacity-90',
